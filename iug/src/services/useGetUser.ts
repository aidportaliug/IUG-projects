import { collection, CollectionReference, DocumentData, getDocs, query, QuerySnapshot, where } from "firebase/firestore";
import { db } from "./firebaseConfig";
import {CustomUser} from "../models/user"

export const  useGetUser = async (userID: string) : Promise<CustomUser | null> => {
    let queryGetUser : CollectionReference<DocumentData>;
    queryGetUser = collection(db, "userProfile")
